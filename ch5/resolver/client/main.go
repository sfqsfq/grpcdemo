...
func callUnaryEcho(c ecpb.EchoClient, message string) {
	...
	defer cancel()
	r, err := c.UnaryEcho(ctx, &ecpb.EchoRequest{Message: message}, grpc.UseCompressor(gzip.Name))
	if err != nil {
...
func (*exampleResolverBuilder) Build(target resolver.Target, cc resolver.ClientConn /* opts resolver.BuildOption */, opts resolver.BuildOptions) (resolver.Resolver, error) {
	r := &exampleResolver{
		target: target,
		cc:     cc,
